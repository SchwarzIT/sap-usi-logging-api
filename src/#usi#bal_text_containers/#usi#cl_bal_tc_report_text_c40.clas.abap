CLASS /usi/cl_bal_tc_report_text_c40 DEFINITION PUBLIC FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_for_program
      ...
      RETURNING VALUE(r_result) TYPE REF TO /usi/cl_bal_tc_report_text_c40.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS get_name_of_calling_program
      RETURNING VALUE(r_result) TYPE programm.
...
CLASS /usi/cl_bal_tc_report_text_c40 IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    textpool_reader = NEW /usi/cl_bal_textpool_reader( ).

    program  = i_program.
    text_key = i_text_key.

    SET LOCALE LANGUAGE sy-langu.
    text_key = to_upper( text_key ).
  ENDMETHOD.

  METHOD create.
    r_result = NEW #( i_program  = get_name_of_calling_program( )
                      i_text_key = i_text_key ).
  ENDMETHOD.

  METHOD create_for_program.
    r_result = NEW #( i_program  = i_program
                      i_text_key = i_text_key ).
  ENDMETHOD.
  ...
  METHOD get_name_of_calling_program.
    ...
    IF sy-subrc <> 0.
      ASSIGN callstack[ 1 ] TO <callstack_line>.
    ENDIF.
