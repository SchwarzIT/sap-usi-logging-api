...
CLASS lcl_unit_tests_serialization IMPLEMENTATION.
  ...
  METHOD test_deserialize_valid_xml.
    ...
    DATA(serialized_data_container) = NEW /usi/cl_bal_dc_json(
                                              i_json_document  = document
                                              i_document_title = title )->/usi/if_bal_data_container~serialize( ).
    DATA(cut) = CAST /usi/cl_bal_dc_json( /usi/cl_bal_dc_json=>/usi/if_bal_data_container~deserialize(
