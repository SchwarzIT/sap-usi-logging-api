...
CLASS /usi/cl_bal_dc_parent_log DEFINITION LOCAL FRIENDS lcl_private_data_getter.

CLASS lcl_private_data_getter DEFINITION FINAL FOR TESTING CREATE PUBLIC.
  PUBLIC SECTION.
    METHODS constructor
      IMPORTING i_data_container TYPE REF TO /usi/cl_bal_dc_parent_log.

    METHODS get_log_handle
      RETURNING VALUE(r_result) TYPE balognr.

  PRIVATE SECTION.
    DATA data_container TYPE REF TO /usi/cl_bal_dc_parent_log.
...
CLASS lcl_unit_tests_serialization IMPLEMENTATION.
  METHOD test_deserialize_bad_json.
    TRY.
        CAST /usi/cl_bal_dc_parent_log( /usi/cl_bal_dc_parent_log=>/usi/if_bal_data_container~deserialize(
                                                `Invalid garbage` ) ).
  ...
  METHOD test_deserialize_valid_json.
    ...
    DATA(serialized_data_container) = NEW /usi/cl_bal_dc_parent_log( input )->/usi/if_bal_data_container~serialize( ).

    DATA(cut) = CAST /usi/cl_bal_dc_parent_log(
                         /usi/cl_bal_dc_parent_log=>/usi/if_bal_data_container~deserialize( serialized_data_container ) ).
...
CLASS lcl_unit_test_cardinality IMPLEMENTATION.
  METHOD assert_is_single_use.
    DATA(actual_result) = /usi/cl_bal_dc_parent_log=>/usi/if_bal_data_container~is_multiple_use_allowed( ).
...
CLASS lcl_unit_test_classname IMPLEMENTATION.
  METHOD assert_returns_right_classname.
    ...
    actual_result   = /usi/cl_bal_dc_parent_log=>/usi/if_bal_data_container~get_classname( ).
