...
CLASS lcl_unit_tests_serialization IMPLEMENTATION.
  ...
  METHOD test_deserialize_valid_xml.
    ...
    DATA(title) = CAST /usi/if_bal_text_container_c40( NEW /usi/cl_bal_tc_literal_c40(
                                                               i_text = 'Test document title' ) ).

    " serialize
    DATA(serialized_data_container) = NEW /usi/cl_bal_dc_html(
                                              i_html_document  = document
                                              i_document_title = title )->/usi/if_bal_data_container~serialize( ).

    " deserialize
    DATA(cut) = CAST /usi/cl_bal_dc_html( /usi/cl_bal_dc_html=>/usi/if_bal_data_container~deserialize(
